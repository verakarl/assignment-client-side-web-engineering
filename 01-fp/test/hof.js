...
describe("01-fp", () => {
  describe("Higher-Order Functions", () => {
    describe("pow(x, y)", () => {
      it.skip("should calc: pow(2, 1)", () => {
        pow(2, 1).should.eql(2);
      });

      it.skip("should calc: pow(2, 2)", () => {
        pow(2, 2).should.eql(4);
      });

      it.skip("should calc: pow(2, 3)", () => {
        pow(2, 3).should.eql(8);
      });

      it.skip("should calc: pow(2, 4)", () => {
        pow(2, 4).should.eql(16);
      });

      it.skip("should calc: pow(2, 128)", () => {
        pow(2, 128).should.eql(3.402823669209385e38);
      });

      it.skip("should throw error: pow(2, 0)", () => {
        should.throws(() => pow(2, 0));
      });

      it.skip("should throw error: pow(2, -1)", () => {
        should.throws(() => pow(2, -1));
      });
    });

    describe("sort(field, primer)", () => {
      it.skip("should sort by age: sort('age')", () => {
      ...
      it.skip("should sort by company: sort('company')", () => {
      ...
      it.skip("should sort by name: sort('name', primer)", () => {
